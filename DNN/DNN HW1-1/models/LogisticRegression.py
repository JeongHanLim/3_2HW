    ...
    def train(self, x, y, epochs, batch_size, lr, optim):
        ...
        for epoch in range(epochs):
            ...
            for i in range(0, x.shape[0]-batch_size, batch_size):
                ...
                sig_y_pred = LogisticRegression.forward(self, x_batch)
                #print(sig_y_pred)
                diff = sig_y_pred - y_batch
                diff = diff.reshape(-1, 1)
                loss += (-y_batch * np.log(sig_y_pred+epsilon) - (1 - y_batch) * np.log(1 - sig_y_pred+epsilon))

                m_grad = np.matmul(x_batch.transpose(), diff).mean(axis=0)
